import type { StitchingItem } from '../../types/stitching';
...
type Props = { items: StitchingItem[]; copy: Copy; onToggleSelected: (sourcePath: string) => void };
...
const labelFor = (item: StitchingItem, copy: Copy) => item.status === 'success' ? copy.statusSuccess : item.status === 'running' ? copy.statusRunning : item.status === 'failed' ? copy.statusFailed : copy.statusReady;
const summaryFor = (item: StitchingItem) => `${item.imageMetadata?.width ?? '-'} × ${item.imageMetadata?.height ?? '-'} · ${(item.imageMetadata?.extension ?? item.sourceName.split('.').pop() ?? 'file').toUpperCase()}`;
const extensionFor = (item: StitchingItem) => (item.imageMetadata?.extension ?? item.sourceName.split('.').pop() ?? 'file').toUpperCase();

const StitchingItemList = ({ items, copy, onToggleSelected }: Props) => (
  <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
    ...
    {items.map((item) => <div key={item.sourcePath} className="grid w-full grid-cols-[32px_52px_1fr_100px] items-center gap-5 rounded-lg px-5 py-4 text-left transition-all duration-300 border border-transparent hover:bg-[#FAFBFD] hover:border-border-light group">
      ...
      <div className="min-w-0"><div className="truncate text-sm font-bold text-[#1A1D23]">{item.sourceName}</div><div className="mt-1 text-[11px] font-medium text-[#8D93A1] flex items-center gap-3"><span>{summaryFor(item)}</span>{item.errorMessage ? <span className="text-meitu truncate flex-1 flex items-center gap-1 font-bold"><span className="w-1 h-1 rounded-full bg-meitu" />{item.errorMessage}</span> : null}</div></div>
...
export default StitchingItemList;
