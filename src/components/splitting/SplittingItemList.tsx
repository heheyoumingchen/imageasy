...
const extensionFor = (item: SplittingItem) => (item.imageMetadata?.extension ?? item.pdfMetadata?.extension ?? item.sourceName.split('.').pop() ?? 'file').toLowerCase();

const getBadge = (item: SplittingItem) => {
  const extension = extensionFor(item);
  return {
    label: extension.toUpperCase(),
    className: badgeClassByExtension[extension] ?? 'bg-[#8F98A8]'
  };
};
      ...
      <div className="flex-1 overflow-y-auto p-4 custom-scrollbar">
        <div className="flex h-full min-h-[320px] flex-col items-center justify-center text-[#8D93A1] animate-in fade-in zoom-in-95 duration-500">
          <div className="mb-4 flex h-20 w-20 items-center justify-center rounded-lg bg-bg-main shadow-inner">
            <svg viewBox="0 0 24 24" className="w-10 h-10 opacity-20" fill="none" stroke="currentColor" strokeWidth="1.5"><path d="M12 5v14M5 12h14" strokeLinecap="round" /></svg>
          </div>
          <p className="px-8 text-center text-sm font-medium opacity-60">{copy.empty}</p>
        </div>
      </div>
    );
  }
    ...
    <div className="flex h-full flex-col">
      <div className="flex-1 overflow-y-auto p-4 space-y-2 custom-scrollbar">
        {items.map((item) => {
          ...
          return (
            <div key={item.sourcePath} className="grid w-full grid-cols-[32px_52px_1fr_100px] items-center gap-5 rounded-lg px-5 py-4 text-left transition-all duration-300 border border-transparent hover:bg-[#FAFBFD] hover:border-border-light group">
              ...
              <div aria-label={`文件类型 ${badge.label}`} className={`flex h-10 w-12 items-center justify-center rounded-lg text-[10px] font-bold text-white shadow-sm transition-transform group-hover:scale-105 ${badge.className}`}>{badge.label}</div>
