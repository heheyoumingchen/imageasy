...
use imageasy_lib::commands::stitching::{
    inspect_stitching_directory, inspect_stitching_file, stitch_image_files,
    StitchImageFilesRequest,
};

fn run_inspect_stitching_file(
    path: String,
) -> Result<imageasy_lib::commands::stitching::InspectStitchingFileResult, String> {
    tauri::async_runtime::block_on(inspect_stitching_file(path))
}

fn run_inspect_stitching_directory(
    path: String,
) -> Result<Vec<imageasy_lib::commands::stitching::InspectStitchingFileResult>, String> {
    tauri::async_runtime::block_on(inspect_stitching_directory(path))
}

fn run_stitch_image_files(
    request: StitchImageFilesRequest,
) -> Result<imageasy_lib::commands::stitching::StitchImageFilesResult, String> {
    tauri::async_runtime::block_on(stitch_image_files(request))
}
...
fn inspect_stitching_directory_collects_images_only() {
    ...
    let result =
        run_inspect_stitching_directory(dir.path().to_string_lossy().into_owned()).unwrap();
...
fn stitch_image_files_outputs_layout_canvas_with_object_cover_fit() {
    ...
    ImageBuffer::<Rgba<u8>, _>::from_pixel(20, 10, Rgba([255, 0, 0, 255]))
        .save(&a)
        .unwrap();
    ImageBuffer::<Rgba<u8>, _>::from_pixel(10, 20, Rgba([0, 0, 255, 255]))
        .save(&b)
        .unwrap();

    let result = run_stitch_image_files(StitchImageFilesRequest {
        cells: vec![
            imageasy_lib::commands::stitching::StitchLayoutCell {
    ...
    let output = image::open(PathBuf::from(&result.output_path))
        .unwrap()
        .to_rgba8();
...
fn stitch_image_files_rejects_too_few_images_and_invalid_columns() {
    let dir = tempdir().unwrap();
    let a = dir.path().join("a.png");
    ImageBuffer::<Rgba<u8>, _>::from_pixel(10, 6, Rgba([255, 0, 0, 255]))
        .save(&a)
        .unwrap();

    let too_few = run_stitch_image_files(StitchImageFilesRequest {
        cells: vec![imageasy_lib::commands::stitching::StitchLayoutCell {
            source_path: a.to_string_lossy().into_owned(),
            row: 0,
            col: 0,
            row_span: 1,
    ...
    let invalid_layout = run_stitch_image_files(StitchImageFilesRequest {
        cells: vec![
            imageasy_lib::commands::stitching::StitchLayoutCell {
                source_path: a.to_string_lossy().into_owned(),
                row: 0,
...
fn stitch_image_files_uses_short_side_resolution_and_background_spacing() {
    ...
    let a = dir.path().join("a.png");
    let b = dir.path().join("b.png");
    ImageBuffer::<Rgba<u8>, _>::from_pixel(40, 40, Rgba([255, 0, 0, 255]))
        .save(&a)
        .unwrap();
    ImageBuffer::<Rgba<u8>, _>::from_pixel(40, 40, Rgba([0, 255, 0, 255]))
        .save(&b)
        .unwrap();
...
fn stitch_image_files_accepts_six_times_cell_scale() {
    ...
    let a = dir.path().join("a.png");
    let b = dir.path().join("b.png");
    ImageBuffer::<Rgba<u8>, _>::from_pixel(10, 10, Rgba([255, 0, 0, 255]))
        .save(&a)
        .unwrap();
    ImageBuffer::<Rgba<u8>, _>::from_pixel(10, 10, Rgba([0, 0, 255, 255]))
        .save(&b)
        .unwrap();
...
fn stitch_image_files_preserves_background_around_rounded_cell_corners() {
    ...
    let a = dir.path().join("a.png");
    let b = dir.path().join("b.png");
    ImageBuffer::<Rgba<u8>, _>::from_pixel(80, 80, Rgba([255, 0, 0, 255]))
        .save(&a)
        .unwrap();
    ImageBuffer::<Rgba<u8>, _>::from_pixel(80, 80, Rgba([0, 0, 255, 255]))
        .save(&b)
        .unwrap();
